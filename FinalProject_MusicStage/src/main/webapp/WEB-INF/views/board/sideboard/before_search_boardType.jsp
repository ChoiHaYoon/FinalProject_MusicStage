			...
			<div class="board_div" data-bno="${ list.board_number}" style="margin-top: 50px;">
				...
				<div class="thumbnail">
					<c:if test="${ boardFile.file_change_name ne '썸네일-기본이미지.gif' }">
						<img class="f_thumbnail" onclick="thumbnail();" src="<%= request.getContextPath() %>/resources/boardImageFile/${ boardFile.file_change_name }">
					</c:if>
					<c:if test="${ boardFile.file_change_name eq '썸네일-기본이미지.gif' }">
						<img class="f_thumbnail" onclick="thumbnail();" src="<%= request.getContextPath() %>/resources/images/썸네일-기본이미지.gif">
					</c:if>
				</div>
