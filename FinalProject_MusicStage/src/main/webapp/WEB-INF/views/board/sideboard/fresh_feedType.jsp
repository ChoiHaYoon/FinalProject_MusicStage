...
<body>
         ...
         if($('#n_select option:selected').val() == "신고내용"){
            ...
            $.ajax({
               ...
               success: function(data){
                  console.log("신고가 돼?" + data);
                  /* if(data[0] > 0){ //이미 신고한 게시물임
                     alert("이미 신고한 게시물입니다.");
                     $("#report_modal").fadeOut("fast").css({"display":"none"});
                     $("#n_text").val('');
                     $('#n_select').val('신고내용');
   ...
   <script>
	...
	window.onload = function(){
		...
		$(".main_feed_table").slice(0, 7).show();
...
//             /* 검은 책갈피를 눌렀다면..보관함 취소 */
	...
	<script>
		...
		function insertreply(e){
			...
			console.log("alarm_receiver : " + alarm_receiver);
			console.log("alarm_receiver_nickName : " + alarm_receiver_nickName);
