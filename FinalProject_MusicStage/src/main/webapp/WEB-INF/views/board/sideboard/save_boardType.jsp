...
<body>
	...
	<section class="section">
		...
		<div id="centerWrap">
			...
			<c:forEach var="boardFile" items="${ list.board.boardFile }"><!-- 그에맞는 BOARD에 맞는 BOARDFILE들 -->
			<!-- start -->
			<div class="board_div" data-bno="${ list.board.board_number}">
				<div class="save_board_code" data-scode="${ list.save_board_code }"></div>
				<!-- conroller로 값넘기기 -->	
				<c:url var="bdetail" value="bDetail.bo">
					<c:param name="bNum" value="${ list.board.board_number }"/>
				</c:url>
				...
				<div class="board_profile_div">
					<div class="profile_link">
						...
						<c:if test="${ empty list.board.member.profile_pic }"> <!-- 프로필사진이 null일경우 -->
						<img class="f_img" src="<%= request.getContextPath() %>/resources/images/profileDefault.png">
						</c:if>
						
						<c:if test="${ !empty list.board.member.profile_pic }">
						<img class="f_img" src="<%= request.getContextPath() %>/resources/userProfile/${ list.board.b_writer }/${ list.board.member.profile_pic }">
						</c:if>
					</div>
					<div class="title_link">
						<a href="<%= request.getContextPath() %>/bDetail.bo?bNum=${ list.board.board_number }" class="f_title" style="font-size: 13px !important;">${ list.board.board_title }</a>
					</div>
				</div>

				<!-- 3. 작성자, 조회수 div -->
				<!-- ///////////////// -->
				<div class="writer">${ list.board.member.nickName }</div>
				<div class="views">조회수 ${ list.board.read_count }</div>
			</div>
			<!-- end -->
			</c:forEach>
			</c:forEach>
 						...
 						success : function(data){
 							...
 							if(data.length != 0){
	 							...
	 							for(i in data){
	 								...
	 								var writer = data[i].board.member.nickName;
