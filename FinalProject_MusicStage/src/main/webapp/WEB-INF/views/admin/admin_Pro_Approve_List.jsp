		...
		<div id="centerWrap"> <!-- 여기에다가 넣으면 됩니다 시작 -->
			...
			<div id="m_content" align=center>
		    	...
		    	function searchBoard(){
		        	...
		        	location.href="searchProApproveList.ad?searchCondition="+searchCondition;
		    	}
			    </script>			    
					...
					<c:forEach var="rp" items="${list}">
						...
						<c:choose>
							<c:when test="${ empty list}">
								<td colspan="7">전문가 등록 요청이 없습니다.</td>
							</c:when>
							<c:otherwise>						
						
								<td class="p_num">${rp.register_no}</td>
								<td class="p_cate">${rp.specialty_field}</td>
								<td class="p_title">${rp.introduce}</td>
								<td class="p_writer">${rp.register_user_nickName}</td>
								<td class="p_date"><fmt:formatDate value="${rp.register_date}" pattern="yyyy-MM-dd" /></td>
								<td class="p_result">${rp.register_approve}</td>
							</c:otherwise>
						</c:choose>
						</tr>
					</c:forEach>
			...
			<div id="paging_area" align= center>
	         	...
	         	<c:if test="${searchCondition eq null}">
	         		<c:set var="loc" value="/proApprovetList.ad" scope="page"/>
	         	</c:if>
	         	<c:if test="${searchCondition ne null}">
	         		<c:set var="loc" value="/searchProApproveList.ad" scope="page"/>
	         	</c:if>  
	...
	<script>
		$(function(){
			$('.content').click(function(){
				var register_no = $(this).children('td').eq(0).text();
				var searchCondition = '${searchCondition}';
				location.href = "proApproveDetail.ad?register_no=" + register_no + "&page=" + ${pi.currentPage} + "&searchCondition=" + searchCondition;
			});
		});
	</script>	
