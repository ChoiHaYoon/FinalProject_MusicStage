...
<style>
	...
	/* 카테고리 선택 시 */
	#b_report{ background: gray; color: white; font-weight: bold; }
	...
	/* 신고 게시글 */
	#n_board{ width: 60px; margin-right: 10px; font-size: 14px; font-weight: bold; vertical-align: middle; display: inline-block; text-align: right;}
	#report_board_no{ width: 400px; text-indent: 10px; background: none; border: none; }
	/* 신고글카테고리 */
		...
		<div id="centerWrap"> <!-- 여기에다가 넣으면 됩니다 시작 -->
			...
			<div id="m_content" align=center>
		    	...
		    	function searchBoard(){
		        	...
		        	location.href="searchBoardReportList.ad?searchCondition="+searchCondition;
		    	}
			    </script>
				...
				<c:forEach var="rb" items="${list}">
					...
					<c:choose>
						<c:when test="${ empty list}">
							<td colspan="7">게시글 신고 내역이 없습니다.</td>
						</c:when>
						<c:otherwise>						
					
							<td class="a_num">${rb.report_no}</td>
							<td class="a_cate">${rb.board_report_category_name}</td>
							<td class="a_title">${rb.report_content}</td>
							<td class="a_writer">${rb.report_from_nickName}</td>
							<td class="a_date"><fmt:formatDate value="${rb.report_date}" pattern="yyyy-MM-dd" /></td>
							<td class="a_result">${rb.report_status}</td>
						</c:otherwise>
					</c:choose>
					</tr>
				</c:forEach>
			...
			<div id="paging_area" align= center>
	         	...
	         	<c:if test="${searchCondition eq null}">
	         		<c:set var="loc" value="/boardReportList.ad" scope="page"/>
	         	</c:if>
	         	<c:if test="${searchCondition ne null}">
	         		<c:set var="loc" value="/searchBoardReportList.ad" scope="page"/>
	         	</c:if>  
			...
			<div id="modal1">
				<div id="see_notify">
					...
					<input type="hidden" id="report_board_writer" value="" name="report_b_writer">
					<input type="hidden" id="report_board_writer_nickName" value="" name="report_b_writer_nickName">
					...
					<div class="n_object">
						<div id="n_board">신고글</div>
						<input type="text" id="report_board_no" value="게시글 번호(제목)" readonly>
					</div>					
					<div class="n_object">
				...
				<script>
					function check(){
						...
						var report_board_origin = $("#report_board_no").val();
						var report_board_no = report_board_origin.substring(report_board_origin.indexOf("[")+1, report_board_origin.indexOf("]"));
						var report_board_writer = $("#report_board_writer").val();
						var report_board_writer_nickName = $("#report_board_writer_nickName").val();
						...
						console.log("report_board_origin 값 : " + report_board_origin);
						console.log("report_board_no 값 : " + report_board_no);
						console.log("report_board_writer 값 : " + report_board_writer);
						...
						if(ck){
							alert("승인처리를 완료하였습니다.");
							location.href="boardReportYes.ad?report_no="+report_no+"&report_board="+report_board_no
				 		 	...
				 		 	// 신고자 알림 DB저장
				 	 	 	var AlarmData = {
				 	 	 			...
				 	 	 			"alarm_type" : "BoardReportYes",
				 	 	 			"alarm_title" : "<b>MusicStage</b> 게시글 신고 요청이 접수되었습니다.",
				 	 	 			"alarm_content" : "<b>"+report_board_writer_nickName+"</b>님의 게시글 " + "<b>" + report_board_origin + "</b>" + " 에 대한 신고 요청이 접수되었습니다. 신고 내역에 따라 해당 게시글과 계정은 패널티가 적용 될 예정입니다.",
				 	 	 			"b_report_no" : report_no,
				 	 	 			"alarm_board_no" : report_board_no
				 	 	 	};						
				 	 	 	...
				 	 	 	$.ajax({
				 	 	 		...
				 	 	 		success : function(data){
				 	 	 			if(data == 1){
				 	 	 				if(socket){
					 	 	 				let socketMsg = "BoardReportYes," + alarm_sender + ","+alarm_receiver+","+report_no; // 소켓에 보낼 메세지
								 	 	 	...
								 	 	 	var AlarmData2 = {
								 	 	 			"alarm_sender" : alarm_sender,
								 	 	 			"alarm_receiver" : report_board_writer,
								 	 	 			"alarm_type" : "BoardReportYes",
								 	 	 			"alarm_title" : "<b>MusicStage</b> 회원님의 게시글에 대한 신고 요청이 접수되었습니다.",
								 	 	 			"alarm_content" : "<b>"+report_board_writer_nickName+"</b>님의 게시글 " + "<b>" + report_board_origin + "</b>" + " 에 대한 신고 요청이 접수되었습니다. 신고 내역에 따라 해당 게시글과 계정은 패널티가 적용 될 수 있으니 이용에 유의하여 주시기 바랍니다.",
								 	 	 			"b_report_no" : report_no,
								 	 	 			"alarm_board_no" : report_board_no
								 	 	 	};
								 	 	 	...
								 	 	 	$.ajax({
								 	 	 		...
								 	 	 		success : function(data){
								 	 	 			if(data == 1){
									 	 	 			if(socket){
									 	 	 				let socketMsg = "BoardReportYes," + alarm_sender + ","+report_board_writer+","+report_no; // 소켓에 보낼 메세지
									 	 	 				console.log("msgmsg : " + socketMsg); // 소켓 메세지 확인 콘솔
									 	 	 				socket.send(socketMsg); // 소켓에 메세지를 보낸다.
									 	 	 			}else{
									 	 	 				alert("소켓이 열려있지 않습니다.");
			...
			<div id=modal2>
				<div id="no_why">
					<div id="modal_title" align="center">게시글신고-거절</div>
				...
				<script>
					function check2(){
						...
						if(ck){
							...
							location.href="boardReportNo.ad?report_no="+report_no+"&report_board="+report_board_no+"&no_select="+no_select
											+"&page="+${pi.currentPage}+"&searchCondition="+"${searchCondition}";
				 		 	...
				 		 	// 신고자 알림 DB저장
				 	 	 	var AlarmData = {
				 	 	 			"alarm_sender" : alarm_sender,
				 	 	 			"alarm_receiver" : alarm_receiver,
				 	 	 			"alarm_type" : "BoardReportNo",
				 	 	 			"alarm_title" : "<b>MusicStage</b> 게시글 신고 요청이 접수되었습니다.",
				 	 	 			"alarm_content" : "<b>"+report_board_writer_nickName+"</b>님의 게시글 " + "<b>" + report_board_origin + "</b>" + " 에 대한 신고 요청이 다음과 같은 사유로 거절되었습니다.<br><br>"+no_select,
				 	 	 			"b_report_no" : report_no,
				 	 	 			"alarm_board_no" : report_board_no
				 	 	 	};						
				 	 	 	...
				 	 	 	$.ajax({
				 	 	 		...
				 	 	 		success : function(data){
				 	 	 			if(data == 1){
					 	 	 			if(socket){
					 	 	 				let socketMsg = "BoardReportNo," + alarm_sender + ","+alarm_receiver+","+report_no; // 소켓에 보낼 메세지
					 	 	 				console.log("msgmsg : " + socketMsg); // 소켓 메세지 확인 콘솔
					 	 	 				socket.send(socketMsg); // 소켓에 메세지를 보낸다.						
					 	 	 			}else{
					 	 	 				alert("소켓이 열려있지 않습니다.");
        		...
        		$("tr.content").click(function(){
        			...
        			$.ajax({
        				...
        				success : function(data){
        					...
        					if(data.length != 0){
        						$("#cate_see").html(data.board_report_category_name);
        						$("#write_see").val(data.report_from_nickName+'['+data.report_from+']');
        						$("#content_see").html(data.report_content);        						
        						$("#report_board_no").val(data.report_board_title+'['+data.report_board+']');        						
        						$("#enroll_date_p").html("신고 시간 : " + data.report_date);
        						$("#report_board_writer").val(data.report_board_writer);
        						$("#report_board_writer_nickName").val(data.report_board_writer_nickName);
